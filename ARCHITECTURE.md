...
│   ├── watchlog.rs      # the bridge↔live-view pipe. CallRecord (serde JSONL, incl roast_id) appended to ~/.ghost/events.jsonl on every bridged call; read_all/read_from (byte-offset tail) + format_watch_line; BlockStats::from_records + format_blocks_report (count desc/label asc, deterministic); recent_block_roast_ids (bounded tail read -> the roast recency window). pure (de)serialize/format/stats, fs binding thin. feeds `ghost watch` + `ghost blocks` + roast recency.
