...
- **real command wrapper live**: CommandWrapper streams real exec output line-by-line (`run_streaming`); `run(dry_run)` collects it. tested via the external suite (the in-module test mod shadows `CommandWrapper` with a stub, so the REAL streaming impl is covered in tests/skeleton.rs, not interceptor.rs's own tests). old Interceptor::start() kept for compat/skeleton. proxy is stub (no bind).
