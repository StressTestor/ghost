...
last updated: 2026-07-09 (shadow mode + data-plane honesty. FIVE commits: (1) `sanitize_cmd` collapses commands to one feed line — multi-line bash/heredocs stop smashing raw \n's into the jsonl (payload kept, not stripped); (2) `BlockCategory::classify` is now REASON-FIRST — sentinel's deny reason is the authority, classified on its own text so an incidental "token"/".env" in a long command can't hijack the flavor (kills the greedy cred-access bug); tool_name is the weak last-resort hint; (3) **real shadow-attack** (src/shadow.rs): on a denied call in `--mode shadow-attack`, ghost probes semantics-preserving evasions (tight-operators/quote-split/backslash-escape/base64-eval) of a COPY through sentinel and flags any that slip past — bypass discovery from real traffic, telemetry only, the real decision is always made on the original bytes first; (4) `ghost blocks` + the watch line surface the shadow bypasses (ranked policy gaps); (5) stdout-always-JSON property test (60 mode×decision×payload combos) + bounded feed rotation at 8 MiB. +21 tests, 114 total, clippy+fmt clean. verified live against the real sentinel: found genuine bypasses — base64-eval evades every category, quote-split/backslash-escape evade the pipe-to-shell + destructive rules.)
